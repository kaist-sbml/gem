...
#
#
# Copyright (C) 2014-2016 Tilmann Weber, Hyun Uk Kim
# The Novo Nordisk Foundation Center for Biosustainability
# Technical University of Denmark
# Section: New Bioactive Compounds
#
...
#            allFastaList.append(">%s\n%s\n" % (gene_id, fasta_seq))
        allFastaList.append(">%s\n%s\n" % (self.seq_record.id, self.seq_record.seq))
        print allFastaList, len(allFastaList)
        return allFastaList
...
#        allFastaList = self.seq_record
        ...
        for i in range(maxChunks):
            ...
            chunkFileName = "{dirname}{sep}input_{seqid}_{chunk_no:05d}.fasta".format(dirname=chunkDirName, sep=os.sep, seqid=self.seq_record.id, chunk_no=i+1)
            try:
...
#                sys.exit(1)
        cmdline = [EFICAzBinary, fastafile]
            
        logging.debug("executing %s in directory %s" % (" ".join(cmdline), chunkDir))
        proc = subprocess.Popen(cmdline, stdin=None, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
